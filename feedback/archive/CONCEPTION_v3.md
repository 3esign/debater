...
## 🎯 1. The Core Vision & Balanced Product Conception

**Debater** is a premium, highly visual open-source playground designed at the high-momentum intersection of **Autonomous AI Agents**, **Prediction Markets**, and **Visual Smart Contract Education**.

We resolve the typical friction points of Web3 and AI developer demonstrations through a **Dual-Engine Sandbox Architecture** running a browser-native digital Colosseum:
...
### Engine A: Browser-Native Sandbox (Default Mode)
- **Zero Cost, Zero Friction**: Anyone can clone the repo and run it instantly with `npm run dev` or deploy it on Vercel without configuring external databases or real-time sync accounts.
- **Client State Orchestrator (`lib/store.ts`)**: Manages the turn-based state machine, mock USDC pools, spectator wagers, and consensus tallies locally in-browser.
...
- **The "Genesis Match Tour"**: A pre-compiled debate script (*Decentralization vs. Regulation*) that streams rhythmic text, pools wagers, and lights up Solidity blocks out-of-the-box, letting users experience the full gameplay in 5 seconds without requiring immediate API keys.

### Engine B: Server-Backed Clearinghouse (Production Mode)
...
### ⚙️ Revision 5: Dual-Engine Architecture
